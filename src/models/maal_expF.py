...
class MultiTaskNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        # without breaking the train_maal.py expected tuple signature.
        return y_cls, y_seg, fused, saliency_maps + [fused]


class MultiTaskLoss(nn.Module):
    # MAAL Exp F: Unconstrained adaptive MAAL + Cam_head style fused loss
    ...
    def forward(self, y_cls_pred, y_cls_true, y_seg_pred, y_seg_true, saliency_maps):
        ...
        if positive_mask.any():
            ...
            fused_up = F.interpolate(fused_map, size=y_seg_true.shape[2:], mode='bilinear', align_corners=True)
            cam_pos_fused = fused_up[positive_mask]
            ...
            for l, s_map in enumerate(individual_maps):
                s_map_up = F.interpolate(s_map, size=y_seg_true.shape[2:], mode='bilinear', align_corners=True)
