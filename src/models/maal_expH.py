...
class MultiTaskNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        # -------------------------------------------------------------------
        # EXPERIMENT H: FEATURE MASKING (RESIDUAL ATTENTION)
        # -------------------------------------------------------------------
        # Transform the fused map into a probability map [0, 1]
        attention_mask = torch.sigmoid(fused)
        
        # Resize attention mask to match the spatial dimensions of f_final
        ...
        # Mask the final features. 
        # We use (1.0 + mask) as a soft spatial attention residual connection.
        # Background gets *1 (unchanged), Crack gets up to *2 (amplified).
        f_masked = f_final * (1.0 + attention_mask_resized)
        
        # The classifier now only "sees" the masked features
        x_cls_feat = self.avgpool(f_masked)
...
class MultiTaskLoss(nn.Module):
    # MAAL Exp H: Exact same clean loss as Exp F/G
