...
class MultiTaskNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        # -------------------------------------------------------------------
        # EXPERIMENT I: FEATURE MASKING (RESIDUAL ATTENTION + STOP-GRADIENT)
        # -------------------------------------------------------------------
        ...
        # Use .detach() to prevent classification loss from corrupting the mask
        f_masked = f_final * (1.0 + attention_mask_resized.detach())
...
class MultiTaskLoss(nn.Module):
    # MAAL Exp I: Exact same clean loss as Exp H
