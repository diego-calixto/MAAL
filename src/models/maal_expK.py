...
class MultiTaskNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        # -------------------------------------------------------------------
        # EXPERIMENT K: DUAL-STREAM + STOP-GRADIENT + EXPLICABILIDADE COM MAIS PESO
        # -------------------------------------------------------------------
        ...
        # Stop-gradient na mascara
        mask_detached = attention_mask_resized.detach()
        
        # Via 1: Contexto Global
        feat_global = self.avgpool(f_final)
        
        # Via 2: Contexto Focado
        feat_focado = self.avgpool(f_final * mask_detached)
        
        # Concatena as duas vias, dando peso dobrado para a via focada
        feat_concat = torch.cat([feat_global, feat_focado * 2.0], dim=1) # 4096 canais
...
class MultiTaskLoss(nn.Module):
    # MAAL Exp K: Exact same clean loss as Exp H
    ...
    if os.path.exists(DATASET_DIR):
        ...
        if len(df) > 0:
            run_maal_training_pipeline(
                ...
                criterion_factory=lambda: MultiTaskLoss(w_cls=1.0, w_seg=1.0, w_align=ALIGNMENT_WEIGHT * 5.0, num_scales=4),
                df=df,
                n_splits=N_SPLITS,
